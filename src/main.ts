...
function step(timestamp: number, buttonInfo: UpgradeButton) {
  ...
  const increment = (elapsed / 1000) * buttonInfo.growthRate * buttonInfo.timesBought;
...
function makeUpgrade(attrs: UpgradeButton){
  ...
  button.addEventListener("click", () => {ActivateUpgrade(attrs);});
  button.addEventListener("mouseover", () => {CheckFunds(button, attrs);}); // mouse check player funds each
  button.addEventListener("mouseout", () => {CheckFunds(button, attrs);}); // time them mouse over or leave the button
...
function DisplayStats(){
  ...
  for (let i = 0; i < upgradeList.length; i++){
    stats.innerHTML += upgradeList[i].label + ": " + upgradeList[i].timesBought + "\n";
